...
async def send_to_channel(
    channel_id: int, *args, thread_id: int | None = None, **kwargs
    ...
    if not channel:
        return None
    if thread_id:
        if not isinstance(channel, disnake.TextChannel) and not isinstance(
            channel, disnake.ForumChannel
        ):
            logger.error(f"channel {channel_id} is not a text channel: {type(channel)}")
            return None
        thread = (
            channel.get_thread(thread_id) if hasattr(channel, "get_thread") else None
        )
...
def get_item_image(
    item_id: str, color: str | None = None, durability: int | None = None
) -> str:
...
def timestamp_to_unix(date_str: str) -> int:
    ...
    if "L" in date_str:
        logger.debug(
            "timestamp detected to be a Long integer, removing L and parsing..."
        )
