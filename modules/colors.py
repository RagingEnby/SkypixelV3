...
DYE_PATTERN = re.compile(
    r"Combinable in Anvil Changes the color of an armor piece to #([0-9A-Fa-f]+)!"
)
ArmorType = Literal["helmet", "chestplate", "leggings", "boots"]
ExoticType = Literal[
    "exotic", "crystal", "fairy", "og_fairy", "spook", "bleached", "glitched"
]
try:
...
DEFAULT_HEXES: dict[str, str] = {
    ...
    for i in ITEMS
    if i.get("id") and i.get("color") and i["material"].startswith("LEATHER_")
}
MISC_HEXES = {
    "Bleached": "A06540",
    "Pure Red": "FF0000",
...
ARMOR_COLORS: dict[
    ArmorType | Literal["other"], dict[str, tuple[float, float, float]]
] = {
    "helmet": {},
    "chestplate": {},
    "leggings": {},
...
for piece, hexes in OG_FAIRY.items():
    for hex_code_ in hexes:
        ARMOR_COLORS[piece][f"OG Fairy #{hex_code_}"] = colorcompare.hex_to_lab(
            hex_code_
        )

for item_ in ITEMS:
    armor_type_ = (
        item_["material"].replace("LEATHER_", "").lower()
        if item_["material"].startswith("LEATHER_")
        else None
    )
    ...
    dye_match = (
        DYE_PATTERN.fullmatch(item_["description"])
        if item_.get("description")
        else None
    )
    if item_["id"].startswith("DYE_") and dye_match:
        ARMOR_COLORS["other"][item_["name"]] = colorcompare.hex_to_lab(
            dye_match.group(1)
        )
...
def get_exotic_type(
    item_id: str, hex_code: str, extra_attributes: dict | None = None
) -> ExoticType | None:
    hex_code = hex_code.upper()
    if (extra_attributes and extra_attributes.get("dye_item")) or is_variant(
        item_id, hex_code
    ):
        return None
...
def find_closest_pieces(armor_type: ArmorType, color: str | int) -> dict[str, float]:
    lab = (
        colorcompare.hex_to_lab(color)
        if isinstance(color, str)
        else colorcompare.int_to_lab(color)
    )
