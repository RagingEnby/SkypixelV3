...
from modules import asyncreqs, colors, mojang, mongodb, parser, utils
...
async def make_auction_embed(
    auction: dict[str, Any], item: dict[str, Any]
) -> disnake.Embed:
    tag = item.get("tag", {})
    ...
    embed = disnake.Embed(
        ...
        or constants.RARITY_COLORS.get(auction.get("tier", "COMMON")),
    )
    embed.set_thumbnail(
        utils.get_item_image(
    ...
    if not auction["bin"]:
        embed.add_field(
            name="Auction End", value=f"<t:{auction['end'] // 1000}:R>", inline=True
        )
...
class AHListener:
    ...
    async def on_poi_auction(auction: dict[str, Any], item: dict[str, Any]):
        rankname = utils.remove_color_codes(
            await ranktracker.get_rankname(auction["auctioneer"])
        )
        embed = await make_auction_embed(auction, item)
        embed.set_author(
            name=rankname,
    ...
    @staticmethod
    async def on_old_item_auction(
        auction: dict[str, Any], item: dict[str, Any], timestamp: int
    ):
        embed = await make_auction_embed(auction, item)
        embed.add_field(
            name="Created At",
    ...
    @staticmethod
    async def on_seymour_auction(
        auction: dict[str, Any], item: dict[str, Any], hex_code: str
    ):
        embed = await make_auction_embed(auction, item)
        embed.add_field(name="Hex Code", value=f"#{hex_code}", inline=True)
    ...
    @staticmethod
    async def on_exotic_auction(
        auction: dict[str, Any],
        item: dict[str, Any],
        exotic_type: colors.ExoticType,
        hex_code: str,
    ):
        embed = await make_auction_embed(auction, item)
        embed.add_field(name="Hex Code", value=f"#{hex_code}")
        embed.add_field(
            name="Exotic Type", value=exotic_type.replace("_", " ").title(), inline=True
        )
        send_tasks = [
            utils.send_to_channel(constants.EXOTIC_AUCTIONS_CHANNEL, embed=embed)
        ]
        thread_id = constants.EXOTIC_AUCTIONS_THREADS.get(exotic_type)
        if thread_id:
            send_tasks.append(
                utils.send_to_channel(
    ...
    async def on_cake_soul_auction(auction: dict[str, Any], item: dict[str, Any]):
        ...
        cake_owner = (
            item.get("tag", {}).get("ExtraAttributes", {}).get("cake_owner") or "None"
        )
        embed.add_field(
            name="Cake Owner", value=utils.remove_color_codes(cake_owner), inline=True
        )
        pings = []
        if (
            auction["bin"]
            and auction["starting_bid"] <= 7_500_000
            and dye_data
            and dye_data["colorName"] == "red"
        ):
        ...
        await utils.send_to_channel(
            constants.CAKE_SOUL_AUCTIONS_CHANNEL, " ".join(pings), embed=embed
        )

    @staticmethod
    async def on_editioned_auction(
        auction: dict[str, Any],
        item: dict[str, Any],
        edition: int,
        timestamp: int | None,
        ...
        embed = await make_auction_embed(auction, item)
        embed.add_field(
            name="Edition", value=f"#{utils.commaize(edition)}", inline=True
        )
        recipient_name = (
            item.get("tag", {}).get("ExtraAttributes", {}).get("recipient_name")
        )
        if recipient_name:
            embed.add_field(
                name="Recipient Name",
...
class AuctionTrackerCog(commands.Cog):
    ...
    async def on_auction(self, auction: dict[str, Any], new: bool = True):
        try:
            ...
            logger.error(
                f"UNICODE DECODE ERROR: {e} {auction['uuid']} --- {auction['item_bytes']}"
            )
            return
        ...
        timestamp = (
            utils.normalize_timestamp(extra_attributes["timestamp"])
            if extra_attributes.get("timestamp")
            else None
        )
        color = item.get("tag", {}).get("display", {}).get("color")
        hex_code = f"{color:06X}"[:6] if color else None
        exotic_type = (
            colors.get_exotic_type(item_id, hex_code)
            if item_id and hex_code and not extra_attributes.get("dye_item")
            else None
        )
        edition = extra_attributes.get(
            "edition", extra_attributes.get("basket_edition")
        )
        ...
        if (
            timestamp
            and timestamp <= 1575910800
            and auction["starting_bid"] <= 100_000_000
        ):
        ...
        if exotic_type and hex_code:
            tasks.append(
                AHListener.on_exotic_auction(auction, item, exotic_type, hex_code)
        ...
        if edition and edition > 0:
            tasks.append(
                AHListener.on_editioned_auction(auction, item, edition, timestamp)
            )
        if tasks:
    ...
    async def on_auction_end(self, auction: dict[str, Any]):
        try:
            ...
            logger.error(
                f"UNICODE DECODE ERROR: {e} {auction['auction_id']} --- {auction['item_bytes']}"
            )
            return
    ...
    async def active_scanner(self) -> float:
        ...
        if page_0["lastUpdated"] != self.last_scanned_active:
            ...
            await asyncio.gather(
                *[
                    self.on_auction(
                        auction=a, new=a.get("start", 0) >= self.last_scanned_active
                    )
                    for a in auctions
                ]
            )
    ...
    async def ended_scanner(self) -> float:
        ...
        if data["lastUpdated"] != self.last_scanned_ended:
            ...
            await asyncio.gather(*[self.on_auction_end(auction=a) for a in auctions])
    ...
    async def main(self):
        while True:
            try:
                wait_for = min(
                    await asyncio.gather(self.active_scanner(), self.ended_scanner())
                )
