...
SpecialRank = Literal[
    "youtube", "staff", "pig_plus_plus_plus", "innit", "events", "mojang", "mcp"
]
...
POI_UUIDS: set[str] = {
    player["id"] for players in requests.get(URL).json().values() for player in players
}
WATCH_LIST: datamanager.JsonWrapper = datamanager.JsonWrapper(
    "storage/rankwatchlist.json"
)
...
async def edit_stat_channels(rank_data: dict[str, list[dict[str, str]]]):
    ...
    rank_data["special"] = [
        player
        for key, players in rank_data.items()
        if key not in constants.RANK_TRACKER_CHANNELS
        for player in players
    ]
    tasks = []
    for rank, players in rank_data.items():
        channels = constants.RANK_TRACKER_CHANNELS.get(
            rank, constants.RANK_TRACKER_CHANNELS["special"]
        )
        for channel_id in channels.keys():
            ...
            if not isinstance(channel, disnake.TextChannel):
                logger.error(
                    f"channel {channel_id} is not a text channel: {type(channel)}"
                )
                continue
...
async def get_rank_lists() -> dict[SpecialRank, list[dict[str, str]]]:
    ...
    POI_UUIDS = {player["id"] for players in data.values() for player in players}
    return data
...
async def get_player_ranks() -> dict[str, dict[str, str | SpecialRank]]:
    ...
    return {
        player["id"]: {"rank": rank, "name": player["name"]}
...
async def send(
    rank: Literal["special"] | SpecialRank, embed: disnake.Embed, ping: bool = True
):
    ...
    async def callback(self, inter: disnake.ModalInteraction):
        ...
        try:
            ...
            return await inter.response.send_message(
                embed=utils.make_error("Invalid Page", "Please enter a valid number"),
                ephemeral=True,
            )
        ...
        await asyncio.gather(self.view._update_buttons(), inter.response.defer())
        with suppress(disnake.NotFound, disnake.HTTPException):
            await self.view.inter.edit_original_message(
                embed=self.view.embeds[self.view.page], view=self.view
            )
...
class RankListView(disnake.ui.View):
    ...
    async def _update_buttons(self):
        ...
        current_embed.set_footer(text=f"Page {self.page + 1}/{len(self.embeds)}")
...
class RankTrackerCog(commands.Cog):
    ...
    @ranks.sub_command(  # type: ignore[reportUnknownReturnType]
        name="update", description="Update's a player's rank in my database"
    )
    ...
    @ranks.sub_command(  # type: ignore[reportUnknownReturnType]
        name="watch", description="Watch a player for rank changes closer than others"
    )
    async def watch(self, inter: disnake.AppCmdInter, player: str):
        try:
            ...
            return await inter.send(
                embed=utils.make_error(
                    "Player not found!", f"I couldn't find the player `{player}`!"
                )
            )
    ...
    @ranks.sub_command(  # type: ignore[reportUnknownReturnType]
        name="counts", description="Get the amount of players with each rank"
    )
    ...
    @ranks.sub_command(  # type: ignore[reportUnknownReturnType]
        name="list", description="Get a list of players with a specific rank"
    )
    async def list(self, inter: disnake.AppCmdInter, rank: SpecialRank):
        ...
        for i, player in enumerate(rank_list):
            ...
            if len(desc) >= 1000:
                ...
                cur = embeds[index]
                desc = cur.description or ""
            desc += utils.esc_mrkdwn(player["name"]) + (
                ", " if i < len(rank_list) - 1 else ""
            )
